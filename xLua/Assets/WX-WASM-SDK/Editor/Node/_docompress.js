...
const Mod = {
    ...
    async startTextureTask(){
        for(let i=0;i<Conf.textureList.length;i++){
            ...
            let src = `${Conf.dst}/webgl-min/Assets/Textures/png/${width}/${path}.png`;
            if(!fs.existsSync(`${Conf.dst}/webgl-min/Assets/Textures/astc/${width}/`)){
                fs.mkdirSync(`${Conf.dst}/webgl-min/Assets/Textures/astc/${width}/`,{ recursive: true });
            }
            await this.astc({
                src,
                dstPath:`${Conf.dst}/webgl-min/Assets/Textures/astc/${width}/${path}`
            });
            if(Conf.useDXT5 && width%4=== 0 && height%4===0){
                if(!fs.existsSync(`${Conf.dst}/webgl-min/Assets/Textures/dds/${width}/`)){
                    fs.mkdirSync(`${Conf.dst}/webgl-min/Assets/Textures/dds/${width}/`,{ recursive: true });
                }
                await this.dxt5({
                    src,
                    dstPath:`${Conf.dst}/webgl-min/Assets/Textures/dds/${width}/${path}`
                });
            }
            if(!Conf.isAstcOnly){

                if(width === height && potList.indexOf(width)>-1){
                    if(!fs.existsSync(`${Conf.dst}/webgl-min/Assets/Textures/pvr/${width}/`)){
                        fs.mkdirSync(`${Conf.dst}/webgl-min/Assets/Textures/pvr/${width}/`,{ recursive: true });
                    }
                    await this.pvrtc({
                        src,
                        dstPath:`${Conf.dst}/webgl-min/Assets/Textures/pvr/${width}/${path}`
                    });
                }
                if(!fs.existsSync(`${Conf.dst}/webgl-min/Assets/Textures/etc2/${width}/`)){
                    fs.mkdirSync(`${Conf.dst}/webgl-min/Assets/Textures/etc2/${width}/`,{ recursive: true });
                }
                await this.etc2({
                    src,
                    dstPath:`${Conf.dst}/webgl-min/Assets/Textures/etc2/${width}/${path}`,
